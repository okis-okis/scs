...
function schedule_menu_page()
{
	...
	<form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" enctype="multipart/form-data">
		<input type="hidden" name="action" value="process_xls_upload">
		<label for="xls_file">Выберите файл XLS:</label>
		<input type="file" name="xls_file" id="xls_file" accept=".xls">
...
function process_xls_upload()
{
	if (isset($_FILES['xls_file']) && $_FILES['xls_file']['error'] == 0) {
		...
		$target_file = $target_dir . 'f.' . $extension;
	...
	// Предотвращаем дополнительную обработку WordPress
	exit;
}
